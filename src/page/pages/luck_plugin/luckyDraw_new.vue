<template>
    <div class="luckyBox">
        <div class="luckyTitle">(新)还可以抽奖次数{{drawTime}}</div>
    ...
    export default {
        ...
        methods: {
            ...
            dealWithRoll() {
                let _t = this;
                _t.times += 1;
                console.log(_t.times);
                if (_t.times > _t.cycleArr.length) {
                    // 清除定时器
                    ...
                    if (_t.times < parseInt(_t.cycle / 5)) {
                        ...
                        if (_t.times > (_t.cycle - (parseInt(_t.cycle / 5)))) {
                            _t.speed += 50;
                        } else {
                            _t.speed = 100;
                        }
                    ...
                    // 循环调用
                    _t.timer = setTimeout(_t.dealWithRoll, _t.speed);
                }
            },
            // 定时器转盘
            activityRoll() {
                ...
                if (_t.times > _t.cycle && _t.prize === _t.index) {
                    // 清除定时器
                    clearTimeout(_t.timer);
                    _t.prize = 0; // 中奖位置初始化
                    _t.times = 0; // 转动次数初始化
                    ...
                    if (_t.times < _t.cycle) {
                        ...
                        if (_t.times > _t.cycle && (_t.prize === 1 && _t.index === 8) || _t.prize === _t.index) {
                            _t.speed += 110;
                        } else {
                            _t.speed += 20;
                        }
            ...
            roll() {
                ...
                let index = 0;
                if (_t.times >= _t.cycleArr.length) {
                    index = _t.times % (_t.cycleArr.length);
                } else {
                    index = _t.times;
                }
